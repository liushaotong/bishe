...
class Flops_constraint_nin(nn.Module):
    def __init__(self, p, kernel_size, out_size, group_size, size_inchannel, size_outchannel, in_channel=3, HN=False,
                 structure=None):
        super(Flops_constraint_nin, self).__init__()
    ...
    def init_total_flops(self):
        ...
        for i in range(len(self.k_size)):
            total_flops += self.k_size[i] * (self.in_csize[i] / self.g_size[i]) * self.out_csize[i] * self.out_size[
                i] + 3 * self.out_csize[i] * self.out_size[i]
    ...
    def print_current_FLOPs(self, input):
        ...
        for i in range(length):
            ...
            sum_flops += self.k_size[3 * i + 1] * (channels / self.g_size[3 * i + 1]) * self.out_csize[3 * i + 1] * \
                         self.out_size[
                             3 * i + 1] + 3 * self.out_csize[3 * i + 1] * self.out_size[3 * i + 1]

            sum_flops += self.k_size[3 * i + 2] * (self.out_csize[3 * i + 1] / self.g_size[3 * i + 2]) * self.out_csize[
                3 * i + 2] * \
                         self.out_size[3 * i + 2] + 3 * self.out_csize[3 * i + 2] * self.out_size[3 * i + 2]
...
class Flops_constraint_resnet(nn.Module):
    def __init__(self, p, kernel_size, out_size, group_size, size_inchannel, size_outchannel, in_channel=3, w=2,
                 HN=False, structure=None, loss_type='log'):
    ...
    def forward(self, input):
        ...
        if self.HN:
            # self.h = self.input.register_hook(lambda grad: grad)
            arch_vector = []  # 结构向量 由27个0-1列表组成，每个列表元素代表了一个要被剪枝的结构
            start = 0
            for i in range(len(self.structure)):
                end = start + self.structure[i]
                input = input.squeeze()
...
class Flops_constraint_mobnet(nn.Module):
    def __init__(self, p, kernel_size, out_size, group_size, size_inchannel, size_outchannel, in_channel=3, weight=2,
                 HN=False, structure=None):
    ...
    def init_total_flops(self):
        ...
        for i in range(len(self.k_size)):
            current_flops = self.k_size[i] * (self.in_csize[i] / self.g_size[i]) * self.out_csize[i] * self.out_size[
                i] + 3 * self.out_csize[i] * self.out_size[i]
...
def get_middle_Fsize_nin(model, input_res=32):
    # size_in = []
    size_out = []
    size_kernel = []
    size_group = []
