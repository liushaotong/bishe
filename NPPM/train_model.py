...
def main():
    ...
    parser.add_argument('--model_name', default='nin', type=str)
    ...
    if args.warmup:
        ...
        Epoch = Epoch + 5
    best_acc = 0
    best_acc, _, __ = valid(0, net, testloader, best_acc, hyper_net=None, model_string=model_name)
    print(scheduler)

    tb_writer = SummaryWriter('./log/train_log')
    train_tags = ['Train-Acc', 'Train-Loss']
    test_tags = ['Valid-Acc', 'Valid-Loss']

    for epoch in range(0, Epoch):
        # scheduler.step()

        scheduler.step()
        train_loss, train_acc=retrain(epoch, net, criterion, trainloader, optimizer, smooth=args.smooth_flag, alpha=args.alpha)
        tb_writer.add_scalar(train_tags[0], train_acc, epoch)
        tb_writer.add_scalar(train_tags[1], train_loss, epoch)
        best_acc, valid_loss, valid_acc = valid(epoch, net, testloader, best_acc, hyper_net=None, model_string=model_name)
        tb_writer.add_scalar(test_tags[0], valid_acc, epoch)
        tb_writer.add_scalar(test_tags[1], valid_loss, epoch)
